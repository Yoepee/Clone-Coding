...
// 판매글 목록 받아오는 내용
export const __getRelationThing = createAsyncThunk(
  "/api/view/categoriylist/{id}",
...
export const relationThing = createSlice({
  name:"relationThing",
  initialState: {
      data: [],
      success: false,
  ...
  extraReducers: {
      [__getRelationThing.pending]: (state) => {
        state.isLoading = true; // 네트워크 요청이 시작되면 로딩상태를 true로 변경합니다.
      },
      [__getRelationThing.fulfilled]: (state, action) => {
        state.isLoading = false; // 네트워크 요청이 끝났으니, false로 변경합니다.
        state.data = action.payload; // Store에 있는 todos에 서버에서 가져온 todos를 넣습니다.
      },
      [__getRelationThing.rejected]: (state, action) => {
