...
export const __getChatContent = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
      try {
          const data =  await axios.get(process.env.REACT_APP_DANG_GEUN+`/api/chat/message/${payload}`, {
            headers: {
...
export const chatContent = createSlice({
  ...
  extraReducers: {
      [__getChatContent.pending]: (state) => {
        state.isLoading = true; // 네트워크 요청이 시작되면 로딩상태를 true로 변경합니다.
      },
      [__getChatContent.fulfilled]: (state, action) => {
        state.isLoading = false; // 네트워크 요청이 끝났으니, false로 변경합니다.
        state.data = action.payload; // Store에 있는 todos에 서버에서 가져온 todos를 넣습니다.
      },
      [__getChatContent.rejected]: (state, action) => {
